...
Game::Game(sf::RenderWindow& window) : window(window),
    ...
    // FIX: SFML's default key-repeat means holding a key down fires
    // repeated KeyPressed events at the OS repeat rate. That was
    // calling jumpSound->play() over and over while Up/Space was held,
    // overlapping many copies of the same sound rapidly -- the
    // "grumbling" noise. Disabling key repeat means KeyPressed only
    // fires once per actual press, no matter how long it's held.
    window.setKeyRepeatEnabled(false);

    milestoneColors = { sf::Color::Yellow, sf::Color::Red, sf::Color::Green,
                         sf::Color::Cyan, sf::Color::Magenta };

    // NOTE: obstacles is a std::deque, not std::vector -- deque
    ...
    // its own independent volume (0-100). All set to 100 for now;
    // freely adjust any individually to rebalance the mix.
    ...
    // one pair for bare ground and one for standing on a vehicle. File
    // names are placeholders -- point these at whatever you name the
    // actual assets.
    ...
    // ==========================================================
    // INTRO CUTSCENE ASSETS -- see the big comment block on the
    // IntroPhase enum in Game.h for the full phase-by-phase rundown.
    // ==========================================================
    ...
    // here, same 0-100 pattern as everything above. Freely rebalance.
    Obstacle::setCarEngineVolume(85.0f); // was 80 -- lowered per request, now peaks at 55 at screen center (curve handles the rest)
    ...
    // MUSIC VOLUME: separate from the SFX above -- lowered from full
    ...
    // both match THIS (not their own native aspect ratio), same
    // reasoning as the original Quit-matching-Restart fix: these are
    // different source images (different word counts/aspect ratios),
    // so matching by width made them wildly different sizes. Height is
    // the dimension that needs to match across all three.
    ...
    // ---- Menu screen graphics (static background + button stack) ----
    {
        ...
        // Full-screen static background -- title + character art baked
        // in, replaces the old live gameplay preview entirely.
        ...
        float totalHeight = (playHalfH * 2) + buttonGap +
                             ...
                             (highScoreHalfH * 2);
        (void)totalHeight; // available if you want to re-center differently later
        ...
        // ABOUT button -- assets/MenuAbout.png (opens an empty overlay
        // for now -- content TBD, add later)
        if (menuAboutLoaded) {
...
void Game::updateCarJumpScene(float deltaTime, float obstacleSpeed) {
    ...
    // final exit back to normal spawning) now waits on the same real
    // position-check, isDownlaneClearFor() -- no more per-vehicle
    // width/gap-pixel math needed here at all.
    ...
    switch (carSceneState) {
        ...
        case CarSceneState::WaitingJeep: {
            ...
            if (carSceneTimer >= gapTime) {
                if (carSceneIncludesVan) {
                    ...
                    // 2-vehicle chain: Jeep was the last one -- wait
        ...
        case CarSceneState::WaitingClear: {
            // Final hand-off back to normal spawning. Used to be a pure
            // timer-based gap (last vehicle's width + a random pixel
            // gap, converted to a time threshold) -- same idea as
            // drone's clearance check now instead: verify the actual
            // ON-SCREEN POSITION of the just-passed Jeep/Van has really
            // cleared enough of the screen (isDownlaneClearFor()) before
            // resuming, rather than trusting a time estimate. This is
            // what makes Cars the "owner" of the downlane -- every
            // OTHER spawn (drone, and this scene handing back control)
            // has to verify the lane is actually clear; normal Car
            // spawning itself never has to check anything.
...
void Game::spawnObstacle() {
    // Chain, Barricade, and TCan are gone entirely (code removed, not
    // just excluded) since none of them fit the new background art / a
    // highway with nothing that ever stops.
    // Tire excluded from spawning for now -- code kept intact below
    // (not deleted) in case it comes back later.
    //
    // Jeep and DownlaneVan REMOVED from normal random spawning --
    // they're scene-exclusive now (see triggerCarJumpScene2/3 below),
    // since at normal random-spawn size (130/150) they can't actually
    // be jumped correctly. Only the scripted scene spawns them, at
    // their proper heightOverride sizes (281/386).
...
void Game::spawnDrone() {
    ...
    // Sound starts HERE, at the moment the drone actually exists --
    // previously started ~1s earlier at arm/countdown time as an
    // intentional warning cue, but that gave away every spawn check
    // even when nothing was coming, so it's now tied to the real thing.
    if (droneLoaded && !droneSoundActive) {
        droneSound.emplace(droneBuffer);
        droneSound->setLooping(true);
        droneSound->setVolume(droneSoundVolume); // full volume immediately -- your own sound file already has its own fade in/out baked in
        droneSound->play();
...
void Game::spawnBoxThrow() {
    // BOX SPAWN POINT -- must actually spawn PAST VIEW_RIGHT_EDGE
    // (900) to be genuinely off-screen when it appears, otherwise it
    // pops into view mid-air instead of visibly entering. The old
    // "50% of the way from the player" math landed around x=525 --
    // well inside the visible screen -- which is exactly what caused
    // it to just appear. boxSpawnEdgeMargin controls how far past the
    // edge it starts -- since the box travels at a constant BOX_SPEED
    // (not a fixed total duration anymore), a bigger margin just means
    // a proportionally longer, but identically-shaped, flight -- it no
    // longer secretly compresses/stretches the whole arc the way a
    // fixed-duration model did.
    float boxSpawnX = VIEW_RIGHT_EDGE + boxSpawnEdgeMargin;

    // Growth scaling: your tuned BOX_SPEED stays EXACTLY as-is at run
    // start (growthFraction = 0 there), and only grows by +30% by the
    // time the world hits its speed cap -- same growth-fraction
    // formula the other obstacles use, computed independently here
    // since spawnBoxThrow() doesn't have access to update()'s local
    // obstacleSpeed variable (same reasoning as the boxLandX/duration
    // comment below already explains for staying in sync).
    ...
    // Was hardcoded to 27.75f (= the OLD 185.0f growth range * 0.15
    // pass-through) -- silently went stale and capped below 1.0 when
    // the growth range was changed to 160.0f. Now derives itself from
    // the SAME real numbers maxGroundSpeed uses, so it can't drift out
    // of sync again if that range gets tuned further.
    ...
    float effectiveBoxSpeed = BOX_SPEED * (1.0f + 0.30f * boxGrowthFraction); // +30% by the speed cap; base BOX_SPEED (your tuned value) untouched at run start
...
void Game::updateLandableObstacles() {
    ...
    for (auto& obs : obstacles) {
        if (obs.getType() == ObstacleType::Car) {
            ...
            // one flat line. FIX: previously, if more than one segment's
            // X-range overlapped near a seam, whichever segment the
            // loop checked LAST simply overwrote effectiveGround --
            // causing random flips/freezes between segment heights.
            // Now we pick whichever valid segment's height is CLOSEST
            // to the player's current feet position, which is a much
            // more stable "which surface am I actually standing on"
            // decision when segments are near each other.
...
void Game::triggerDeath(const Obstacle* killer, bool hitFromFront) {
    ...
    // normal distance, same as before this change.
    if (foundStop) {
    ...
    // else (Car, Jeep, Van, ThrowingCar, etc.) still uses
    // the generic hitSound, unchanged.
    ...
    // BUG FIX: update() returns early for Dying/GameOver, before ever
    // reaching the milestone-blink code (which only runs in the
    // Playing branch further down). Dying mid-blink used to leave
    // scoreText frozen at whatever color/scale it had at that exact
    // instant, forever -- the reset-to-gold code never got a chance
    // to run. Ending the blink right here, at the moment of death,
    // means it's already back to normal before anything ever draws
    // the frozen death screen.
    milestoneBlinking = false;
    milestoneBlinkTimer = 0.0f;
    if (fontLoaded) {
        scoreText->setFillColor(GOLD_COLOR);
...
void Game::startGame() {
    ...
    // after every restart), per your spec. Reuses the intro's agent
    // sprite slots since the cutscene is already over by this point.
...
void Game::startIntro() {
    ...
    // NOTE: title sound no longer plays here -- it now fires mid-fade-in,
    // once the fade crosses introTitleSoundTriggerFraction (see
    // updateIntro's TitleCard case).
}
...
void Game::updateIntro(float deltaTime) {
    ...
    switch (introPhase) {
        case IntroPhase::TitleCard: {
            ...
            // Title sound fires once the fade-IN crosses the halfway
            // point (or whatever introTitleSoundTriggerFraction is set
            // to) -- not at time 0, per your spec.
        ...
        case IntroPhase::Chase: {
            ...
            // "Hey stop!" -- POSITION-triggered, not timer-triggered:
            // fires once the player has run introHeyStopTriggerDistance
            // px past his OWN spawn point (introPlayerSpawnX) -- distance-
            // from-spawn, not an absolute screen-fraction, so lowering it
            // isn't silently swallowed by however far off-screen he
            // starts. Background does NOT switch yet -- it stays calm
            // (bg1) through the whole shout, only changing once the
            // shout actually finishes (see below), per your spec.
            ...
            if (introHeyStopTriggered && !introBeepsStarted) {
                ...
                if (introFlickerTimer >= introHeyStopDurationSeconds) {
                    // The shout has genuinely finished -- background
                    // switch AND the first beep happen TOGETHER, right
                    // here, not before.
                ...
                introFlickerTimer += deltaTime;
                // Back to a single fixed beat for both on/off, like the
                // original front-flip-era version -- fast and punchy.
                // The beep sound plays whenever bg switches to 3, same
                // as always, but can get cut short if it's longer than
                // introFlickerBeatDuration; that trade-off is intentional
                // here, per your call.
                if (introFlickerTimer >= introFlickerBeatDuration) {
                    ...
                    if (introCurrentBg == 3) {
                        ...
                        // buys back some of the time the shout+gap used to cost.
...
void Game::renderIntro() {
    ...
    // actually begins -- this is what keeps the warehouse a genuine
    // surprise instead of leaking through a semi-transparent black
    // during the fade-in. Once the fade-out starts, it's drawn and
    // revealed as the black clears, exactly as before.
    ...
    if (introBgLoaded && !titleCardHidingBackground) {
        ...
        // Fit-to-800px-width baseline, then your own eyeballed scale/
        // position on top (introBgScaleMultiplier/introBgPosX/Y, Game.h).
...
void Game::update(float deltaTime) {
    ...
    // RUN ANIMATION SPEED-UP: the leg-cycle plays back slightly faster
    // as the game speeds up over time, matching the increasing pace
    // visually. Kept SUBTLE on purpose (small growth rate, modest
    // cap) -- this is cosmetic only and shouldn't look like the
    // player's legs are frantically spinning. Tune both numbers freely.
    ...
    if (droneSpawnCountdown >= 0.0f) {
        ...
        if (isDownlaneClearForDrone()) {
            ...
            droneSpawnCountdown = -1.0f;
            // Countdown cancelled before the drone actually appeared.
            // Sound no longer starts this early (see spawnDrone()), so
            // droneSoundActive should already be false here in normal
            // play -- this is just a safety net in case that ever
            // changes, so a cancelled countdown can never leave the
            // sound playing over nothing.
    ...
    // shape as Drone above, just without a sound-cue countdown (no
    // warning cue was asked for here). Once triggered, spawnBoxThrow()
    // sets boxThrowActive=true, which also pauses normal Car spawning
    // (see the `&& !boxThrowActive` guard above) for the short window
    // between the box appearing and its paired ThrowingCar spawning --
    // otherwise a regular Car could spawn into the same lane a moment
    // before the ThrowingCar does and the two would overlap.
    ...
    if (boxThrowSpawnPending && !boxThrowActive && isDownlaneClearFor(boxThrowClearScreenFraction) && !droneSoundActive && droneSpawnCountdown < 0.0f && !tireActive && tireSpawnCountdown < 0.0f) {
        // FAIRNESS: same reasoning as Drone's block above -- fair random
    ...
    if (boxThrowActive) {
        ...
        if (boxThrowElapsedTimer >= boxThrowActualFlightDuration * effectiveBoxThrowCarSpawnFraction) {
            ...
            boxThrowActive = false;
            // BUG FIX: boxThrowActive flipping false here used to let a
            // normal Car spawn in this exact same frame (spawnTimer had
            // been accumulating unblocked this whole time), landing
            // right on top of the ThrowingCar that had just spawned one
            // line above. Needs its own short block, same reasoning as
            // dronePostSpawnCarBlockTimer below.
    ...
    if (tireSpawnPending && tireSpawnCountdown < 0.0f && isDownlaneClearFor(tireClearScreenFraction) && !boxThrowActive && !droneSoundActive && droneSpawnCountdown < 0.0f) {
        // FAIRNESS: same reasoning as Drone/Box's blocks above -- fair
    ...
    // ---- DRONE SOUND: offscreen cutoff (volume envelope removed --
    // your own sound file already handles fade in/out) ----
...
void Game::renderMenu() {
    ...
    // ---- About overlay: empty panel for now, content TBD ----
    if (showAboutOverlay) {
...
void Game::renderGameplay() {
    ...
    // Debug collision-line drawing (all types) removed per request --
    // fully polished, no longer needed.

    // HUD/UI BELOW THIS POINT: switch to the window's default,
    // unzoomed view. gameView (set by render() before this function
    // is called) is wider than the window on purpose (the zoom-out
    // effect), so anything still drawn under it -- like scoreText's
    // fixed {20,20} position -- lands wherever THAT view maps those
    // coordinates to, not the window's actual top-left corner. This
    // is what was pushing "Score:" away from the corner. Switching to
    // the default view here means every UI element added below (in
    // this function and in whatever calls it, e.g. the Paused dim
    // overlay and the whole Game Over screen) draws in real window
    // pixels and lines up with where its hitbox rect actually is.
    window.setView(window.getDefaultView());
