...
// since Y grows downward on screen). Negative slopes up to the right,
// positive slopes down to the right.
struct RotatedRect {
    sf::Vector2f center;
...
// A round collision shape, for obstacles that are naturally circular
// (e.g. a rolling tire), where a rectangle hitbox would be a poor fit
// compared to an actual circle.
struct Circle {
    sf::Vector2f center;
    float radius = 0.0f;
};
